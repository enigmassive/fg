...
pub fn statement(
    ...
    let declaration = declaration().map(Statement::Declaration);
...
pub fn declaration() -> impl Parser<Token, Declaration, Error = Simple<Token>> {
    let var_decl = just(Token::Keyword(Keyword::Var))
        .ignore_then(name())
        .then(r#type())
        .then(just(Token::Symbol(Symbol::Equal)))
        .then(expression())
        .map(|(((name, r#type), _), expr)| Declaration::VarDecl(name, r#type, expr));
...
pub fn if_stmt(
    ...
    recursive(|r#if| {
        let var_decl = name()
            .then(just(Token::Symbol(Symbol::ColonEqual)))
            .then(expression())
            .then_ignore(just(Token::Symbol(Symbol::Semicolon)))
            .map(|((name, _), expr)| ShortVarDecl(name, expr));
        ...
        let r#else = just(Token::Keyword(Keyword::Else))
            ...
            .map(ElseStmt::Else);
        let else_stmt = else_if.or(r#else);

        just(Token::Keyword(Keyword::If))
            .ignore_then(var_decl.or_not())
            .then(expression())
            .then(block.clone())
            .then(else_stmt.or_not())
            .map(|(((var_decl, condition), block), else_stmt)| IfStmt {
                var_decl,
                condition,
                block,
                else_stmt,
